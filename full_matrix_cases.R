...
adjs <- sort(list.files(path="~/TCGA_miRNA_BC/miRNAs_resultados/miRNAS_86_resultados/casos_resultados_ARACNE", pattern="*.adj"))
...
write.table(enfermos_norm, file = "enfermos_p1_full_norm_adjmtx.txt", sep = "\t", col.names= NA, row.names= T, quote = F)
xg_norm <- graph.adjacency(adjmatrix= enfermos_norm, mode='undirected', diag=F, weighted=T)
xsif_norm <- get.data.frame(xg_norm)
write.table(xsif_norm, file = "enfermos_p1_full_norm_sif.txt", sep = "\t", col.names= T, row.names= F, quote = F )

mirnas <- enfermos[mir, mir]
write.table(mirnas, file = "enfermos_mir_mir_p1_adjmtx.txt", sep = "\t", col.names= NA, row.names= T, quote = F)
xgmm <- graph.adjacency(adjmatrix= mirnas, mode='undirected', diag=F, weighted=T)
xsifmm <- get.data.frame(xgmm)
write.table(xsifmm, file = "enfermos_mir_mir_p1_sif.txt", sep = "\t", col.names= T, row.names= F, quote = F )

mirnas_norm <- enfermos_norm[mir, mir]
write.table(mirnas_norm, file = "enfermos_mir_mir_p1_norm_adjmtx.txt", sep = "\t", col.names= NA, row.names= T, quote = F)
xgmm_norm <- graph.adjacency(adjmatrix= mirnas_norm, mode='undirected', diag=F, weighted=T)
xsifmm_norm <- get.data.frame(xgmm_norm)
write.table(xsifmm_norm, file = "enfermos_mir_mir_p1_norm_sif.txt", sep = "\t", col.names= T, row.names= F, quote = F )

gengen <- enfermos[genes, genes]
write.table(gengen, file = "enfermos_gen_gen_p1_adjmtx.txt", sep = "\t", col.names= NA, row.names= T, quote = F)
xgg <- graph.adjacency(adjmatrix= gengen, mode='undirected', diag=F, weighted=T)
xsifgg <- get.data.frame(xgg)
write.table(xsifgg, file = "enfermos_gen_gen_p1_sif.txt", sep = "\t", col.names= T, row.names= F, quote = F )

gengen_norm <- enfermos_norm[genes, genes]
write.table(gengen_norm, file = "enfermos_gen_gen_p1_norm_adjmtx.txt", sep = "\t", col.names= NA, row.names= T, quote = F)
xgg_norm <- graph.adjacency(adjmatrix= gengen_norm, mode='undirected', diag=F, weighted=T)
xsifgg_norm <- get.data.frame(xgg_norm)
write.table(xsifgg_norm, file = "enfermos_gen_gen_p1_norm_sif.txt", sep = "\t", col.names= T, row.names= F, quote = F )
...
write.table(mir_gen, file = "enfermos_mir_gen_p1_adjmtx.txt", sep = "\t", col.names= NA, row.names= T, quote = F)
xmg <- graph.adjacency(adjmatrix= mir_gen, mode='undirected', diag=F, weighted=T)
xsifmg <- get.data.frame(xmg)
write.table(xsifmg, file = "enfermos_mir_gen_p1_sif.txt", sep = "\t", col.names= T, row.names= F, quote = F )

mir_gen_norm <- enfermos_norm
mir_gen_norm[genes,genes] <- NA
mir_gen_norm[mir,mir] <- NA
mir_gen_norm[is.na(mir_gen_norm)] <- 0
isSymmetric(mir_gen_norm)

write.table(mir_gen_norm, file = "enfermos_mir_gen_p1_norm_adjmtx.txt", sep = "\t", col.names= NA, row.names= T, quote = F)
xmg_norm <- graph.adjacency(adjmatrix= mir_gen_norm, mode='undirected', diag=F, weighted=T)
xsifmg_norm <- get.data.frame(xmg_norm)
write.table(xsifmg_norm, file = "enfermos_mir_gen_p1_sif.txt", sep = "\t", col.names= T, row.names= F, quote = F )
